# 1 "Source/Ghost/ghost.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 393 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "Source/Ghost/ghost.c" 2
...
extern volatile int mazeGrid[29][28];
...
// int colDiff = targetCol - ghost->col;
  ...
  if (ghost->isChasing) {
    ...
    if (rowDiff != 0) {
        ...
        if (tryRow >= 0 && tryRow < 29 && mazeGrid[tryRow][ghost->col] != 1) {
            *nextRow = tryRow;
            *nextCol = ghost->col;
            return;
        }
    ...
    if (colDiff != 0) {
        ...
        // Check if move is valid
        if (tryCol >= 0 && tryCol < 28 && mazeGrid[ghost->row][tryCol] != 1) {
            *nextRow = ghost->row;
            *nextCol = tryCol;
            return;
        }
...
void updateGhost(void)
{
    ...
    if (oldCellType == 2) {
        fillCell(oldRow, oldCol, offsetX, offsetY, 0x0000);
        // standard small pill
        drawPill(oldRow, oldCol, offsetX, offsetY, 0xFFE0, 1);
    }
    else if (oldCellType == 3) {
        fillCell(oldRow, oldCol, offsetX, offsetY, 0x0000);
        // bigger pill
        drawPill(oldRow, oldCol, offsetX, offsetY, 0xF800, 3);
    }
    else {
        // just empty floor
        fillCell(oldRow, oldCol, offsetX, offsetY, 0x0000);
...
void handleGhostTimer(void)
{
  if (!blinky.isActive) {
    ...
    if (blinky.respawnTimer <= 0) {
      ...
      blinky.row = 13;
      blinky.col = 14;
      blinky.isChasing = 1; // ghost reverts to normal chase
    }
  }
