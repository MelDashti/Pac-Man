...
static _Bool effectPlaying = 0; // Flag for sound effect state
...
void playBackgroundMusic(NOTE* song, int length) {
    ...
    bgSongLength = length;
    bgNoteIndex = 0;
    effectPlaying = 0; // Ensure no effect is playing
        ...
        if (ticks == 1) {
            ...
            if (effectPlaying) {
                ...
                if (effectNoteIndex >= effectLength) {
                    effectPlaying = 0; // Stop effect playback
                    effectNoteIndex = 0; // Reset effect index
                }
            } else if (currentBackgroundSong != 0) {
